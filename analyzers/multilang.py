...
def _check_json_file(path: str, content: str, lines: list[str], sev: dict) -> list[dict]:
    findings: list[dict] = []
    try:
        json.loads(content)
...
def _check_yaml_file(path: str, lines: list[str], sev: dict) -> list[dict]:
    findings: list[dict] = []
    for idx, line in enumerate(lines, 1):
        if "\t" in line and (line.startswith("\t") or re.match(r"^\s*\t", line)):
            ...
            findings.append({
                ...
                "description": (
                    "YAML specification does not allow tab characters for indentation. "
                    "Tabs cause YAML parsers to fail."
                ),
...
def _check_toml_file(path: str, content: str, sev: dict) -> list[dict]:
    findings: list[dict] = []
    if tomllib is None:
        return findings
    try:
...
def _check_js_ts_file(path: str, lines: list[str], sev: dict) -> list[dict]:
    findings: list[dict] = []
    for idx, line in enumerate(lines, 1):
        stripped = line.strip()
        ...
        if m_eq:
            ...
            findings.append({
                ...
                "description": (
                    "Loose equality (`==` and `!=`) performs implicit type coercion, "
                    "leading to unexpected truthiness bugs."
                ),
        ...
        if m_dbg:
            ...
            findings.append({
                ...
                "description": (
                    "Debugger statements pause code execution in browser/node debuggers "
                    "and should not ship to production."
                ),
...
def _check_html_file(path: str, lines: list[str], sev: dict) -> list[dict]:
    findings: list[dict] = []
    for idx, line in enumerate(lines, 1):
        ...
        if HTML_EMPTY_HREF.search(line):
            ...
            findings.append({
                ...
                "description": (
                    "Anchors with `#` href cause page scrolling to top. "
...
def _check_css_file(path: str, lines: list[str], sev: dict) -> list[dict]:
    findings: list[dict] = []
    for idx, line in enumerate(lines, 1):
...
def _check_dockerfile(path: str, lines: list[str], sev: dict) -> list[dict]:
    findings: list[dict] = []
    has_user = False
    for idx, line in enumerate(lines, 1):
        ...
        if m_lat:
            ...
            findings.append({
                ...
                "description": (
                    "Using `:latest` or omitting image tags causes non-deterministic "
                    "builds and breaking upgrades."
                ),
                "evidence": get_snippet(lines, idx),
                "confidence": "high",
                "fix_suggestion": (
                    f"Pin image to a specific version or digest (e.g. `FROM {m_lat.group(1)}:1.2-slim`)."
                ),
            })
        ...
        if m_sens:
            ...
            findings.append({
                ...
                "description": (
                    "Secrets, keys, or .env files copied into Docker images leak "
                    "through layers and public registries."
                ),
                "evidence": get_snippet(lines, idx),
                "confidence": "high",
...
def _check_shell_file(path: str, lines: list[str], sev: dict) -> list[dict]:
    findings: list[dict] = []
    ...
    if not has_set_e:
        ...
        findings.append({
            ...
            "description": (
                "Shell scripts without `set -e` continue running after failed commands, "
    ...
    for idx, line in enumerate(lines, 1):
        ...
        if m_rm:
            ...
            findings.append({
                ...
                "description": (
                    f"If `${m_rm.group(1)}` is empty or unset, this executes `rm -rf /`, "
                    "destroying the filesystem."
                ),
                "evidence": get_snippet(lines, idx, m_rm.start()),
                "confidence": "high",
                "fix_suggestion": (
                    f'Quote the variable: `rm -rf "${m_rm.group(1)}"` and check variable is set first.'
                ),
            })
    return findings
...
def analyze_multilang(path: str, rules: dict) -> list[dict]:
    ...
    if ext == ".json":
        return _check_json_file(path, content, lines, sev)
    if ext in (".yaml", ".yml"):
        return _check_yaml_file(path, lines, sev)
    if ext == ".toml":
        return _check_toml_file(path, content, sev)
    if ext in (".js", ".jsx", ".ts", ".tsx", ".mjs", ".cjs"):
        return _check_js_ts_file(path, lines, sev)
    if ext in (".html", ".htm"):
        return _check_html_file(path, lines, sev)
    if ext in (".css", ".scss"):
        return _check_css_file(path, lines, sev)
    if fname in ("dockerfile", "dockerfile.dev", "dockerfile.prod") or ext == ".dockerfile":
        return _check_dockerfile(path, lines, sev)
    if ext in (".sh", ".bash"):
        return _check_shell_file(path, lines, sev)
